...
$error = "";
			...
			if($_SERVER['REQUEST_METHOD'] == 'POST' && !empty($_POST["ip"]) && !empty($_POST["option"]) && !empty($_POST["net_ip"]) && !empty($_POST["type"]))
			{
				$ip = $_POST["ip"];
				$option = $_POST["option"];
				$net_ip = $_POST["net_ip"];
				...
				dns_delete_ip_array_option($type, $option, $net_ip, "../tmp_file/service_dns".$ip);
