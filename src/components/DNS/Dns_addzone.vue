	...
	export default {
		...
		methods: {
			...
			initForm() {
				...
				if (this.allow_query === true) {
					fields.fields['zoneallowquery'] = {
						rules: [{type   : 'regExp[^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$]', // A APPROFONDIR
						prompt : 'Veuillez remplir correctement le champs {name}'}]
					};

					for (var i = 0; i < this.bonus_zone_allow_query.length; i++) {
						fields.fields['allow_query_' + i] = {
							rules: [{type   : 'regExp[^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$]', // A APPROFONDIR
							prompt : 'Veuillez remplir correctement le champs {name}'}]
						};
					}
				}
        if (this.forwarders === true) {
          fields.fields['zoneforwarders'] = {
            rules: [{type   : 'regExp[^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$]', // A APPROFONDIR
            prompt : 'Veuillez remplir correctement le champs {name}'}]
          };

          for (var i = 0; i < this.bonus_zone_forwarders.length; i++) {
            fields.fields['forwarders_' + i] = {
              rules: [{type   : 'regExp[^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$]', // A APPROFONDIR
              prompt : 'Veuillez remplir correctement le champs {name}'}]
            };
          }
      ...
      removeOneforwarders(index) {
        ...
        if (this.forwarders) {
          ...
          newconf.forwarders = forwarders_array;
				...
				(response) => {
					console.log("error addZoneDns - ", response);
					this.loading = false;
				});
			}
