	...
	export default {
		...
		data () {
			return {
				...
				name: this.$route.params.host.name,
				hardethernet: this.$route.params.host.hardwareethernet,
				ipfixe: this.$route.params.host.fixedaddress,
			};
		},
		...
		methods: {
			...
			initForm() {
				...
				console.log(this.$route.params.subnet_ip);
			...
			addHost() {
				...
				this.$http.post('dhcp/subnets/hc/edit', {"ip": this.get_servers[this.$route.params.id].ip, "subnet_ip": this.$route.params.subnet_ip, "host-client": host_conf} ).then((response) => {
					this.loading = false;
