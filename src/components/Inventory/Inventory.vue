<template>
	<div class="ui segment">
      ...
      <div class="ui horizontal divider" style="padding-top: 50px; padding-bottom: 50px">
		...
		<a class="ui green button middle aligned" :href="serviceUrl"><h3 style="margin: 1rem;">Accèder à l'IHM du service Inventaire</h3></a>
    ...
    methods: {
			...
			installingAgent() {
				if ($('#installAgent').form("is valid")) {
					...
					this.$http.post('inventory/agents/install', {"ip" : this.get_servers[this.$route.params.id].ip, "agent_ip": this.ipaddress} ).then((response) => {
						$("#confirm_add_agent").toggleClass( "loading" );
					},
					(response) => {
			...
			getUrl() {
				...
				this.url = "";
				this.$http.post('inventory/url', {"ip": this.get_servers[this.$route.params.id].ip } ).then((response) => {
