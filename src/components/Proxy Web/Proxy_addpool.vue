<template>
	<div class="ui segment">
		<div class="ui two column stackable centered grid container">
			...
			<form v-on:submit.stop.prevent="" class="ui form column" v-bind:class="{ 'loading': loading }" id="addnewpoolproxy" style="padding-bottom: 50px">
				...
				<div class="ui stacked secondary">
					...
					<div class="required field">
						...
						<div class="ui left icon input">
							<i class="server icon"></i>
							<input type="text" name="pool_name" id="pool_name" v-model="pool_name" placeholder="Nom du pool">
						</div>
					</div>
				</div>
        ...
        let pool_info = {
          ...
          "option": [
              ...
              "forwardfor"
				...
				this.$http.post('proxy/pools/add', {"ip": this.get_servers[this.$route.params.id].ip, "pool_info": pool_info} ).then((response) => {
					this.loading = false;
					this.$router.push({ name: 'proxy' });
					console.log(response);
				},
				(response) => {
					console.log("error addPool - ", response);
					this.loading = false;
				});
			}
