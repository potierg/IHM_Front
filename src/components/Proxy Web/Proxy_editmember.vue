	...
	<div class="ui segment">
		<div class="ui two column stackable centered grid container">
			<div class="ui horizontal divider" style="padding-top: 50px; padding-bottom: 50px">
				Edition du membre "{{memberName}}" du pool "{{poolName}}"
			</div>

			<form v-on:submit.stop.prevent="" class="ui form column" v-bind:class="{ 'loading': loading }" id="editpoolmemberproxy" style="padding-bottom: 50px">
				...
				<div class="ui stacked secondary">
					...
					<div class="required field">
						...
						<div class="ui left icon input">
							<i class="server icon"></i>
							<input type="text" name="member_name" id="member_name" v-model="member_name" placeholder="Nom du membre" disabled="disabled">
						</div>
					</div>
				</div>
	...
	export default {
		...
		data () {
          ...
          rules: [{type   : 'empty', // A APPROFONDIR
				...
				ipaddress: this.$route.params.info.member_ip,
				member_name: this.$route.params.info.member_name,
			};
		},
		...
		methods: {
			...
			initForm() {
				$('#editpoolmemberproxy').form('destroy');
				let _this = this;
				...
				$('#editpoolmemberproxy').form(fields);
			},
        ...
        let req = {
          ...
          "name_server": this.$route.params.info.member_name,
          "new_ip": this.ipaddress,
          "name_pool": this.$route.params.info.pool_name
        };

				this.$http.post('proxy/pools/member/update', req ).then((response) => {
					this.loading = false;
					this.$router.push({ name: 'proxy' });
					console.log(response);
				},
				(response) => {
					console.log("error editpoolMember - ", response);
					this.loading = false;
				});
			}
      ...
      memberName: function() {
        return this.$route.params.info.member_name;
      }
		},
